...
type InlineVolumeTestDriver interface {
	...
	GetVolumeSource(readOnly bool, fsType string, volumeInfo interface{}) *v1.VolumeSource
}
...
type PreprovisionedPVTestDriver interface {
	...
	// It will return nil, if the TestDriver doesn't support either of the parameters.
	GetPersistentVolumeSource(readOnly bool, fsType string, volumeInfo interface{}) *v1.PersistentVolumeSource
}
...
func DeleteVolume(driver TestDriver, volType testpatterns.TestVolType, volumeInfo interface{}) {
	...
	case testpatterns.PreprovisionedPV:
		if pDriver, ok := driver.(PreprovisionedVolumeTestDriver); ok {
			pDriver.DeleteVolume(volType, volumeInfo)
		}
	case testpatterns.DynamicPV:
