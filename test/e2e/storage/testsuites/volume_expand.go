...
func (v *volumeExpandTestSuite) defineTests(driver TestDriver, pattern testpatterns.TestPattern) {
	...
	ginkgo.BeforeEach(func() {
		...
		if !driver.GetDriverInfo().Capabilities[CapExpansion] {
	...
	cleanup := func() {
		...
		if l.pod2 != nil {
			...
			gomega.Expect(err).ToNot(gomega.HaveOccurred(), "while deleting pod2")
			l.pod2 = nil
		}
	...
	if !pattern.AllowExpansion {
		ginkgo.It("should not allow expansion of pvcs without AllowVolumeExpansion property", func() {
			...
			e2elog.Logf("currentPvcSize %v", currentPvcSize)
		...
		ginkgo.It("Verify if editing PVC allows resize", func() {
			...
			pvcClaims := []*v1.PersistentVolumeClaim{l.resource.pvc}
			l.pod, err = framework.CreatePod(f.ClientSet, f.Namespace.Name, nil, pvcClaims, false, "")
			...
			l.pod2, err = framework.CreatePod(f.ClientSet, f.Namespace.Name, nil, pvcClaims, false, "")
